...
func (g *Group) Eval(ctx context.Context, ts time.Time) {
	...
	for i, rule := range g.rules {
		...
		eval := func(i int, rule Rule, cleanup func()) {
			...
			if g.lastEvalTimestamp.IsZero() || g.shouldRestore {
				if ar, ok := rule.(*AlertingRule); ok {
					restoredAlerts := g.alertStore.GetAlerts(ar.GetAlertFingerprint())
					if restoredAlerts != nil {
						ar.SetActiveAlerts(restoredAlerts)
						level.Info(g.logger).Log("msg", "Restored alerts from file", "restoredAlerts", len(restoredAlerts), "rule", rule.String(), "length of active alerts", len(ar.active), "ar.active", ar.active)
					}
				}
			}
